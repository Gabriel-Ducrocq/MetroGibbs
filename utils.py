...
def compare(conj_path, crank_path, mala_path, index):
    ...
    plt.show()
    plt.close()
    plt.hist(path_cls_conj[1000:, index], bins=100, density = True, alpha=0.2, label="CG")
    plt.hist(path_cls_crank[1000:, index], bins=100, density=True, alpha=0.2, label="CN")
    plt.hist(path_cls_mala[1000:, index], bins=100, density=True, alpha=0.2, label="MALA")
    plt.axvline(x=realized_cls[index], color='k', linestyle='dashed', linewidth=1)
    plt.axvline(x=true_cls[index], color='k', linewidth=1)
